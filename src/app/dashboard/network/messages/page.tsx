...
export default function HrPrepPage() {
  ...
  const hrApps = applications.filter((a: any) => ['applied', 'hr_screen'].includes(a.status));

  const handleGeneratePrep = async (app: any) => {
    if (!limits.has_interview_prep) return;
    setAiPrepApp(app); setAiPrepLoading(true); setAiPrepError(null); setAiPrep(null);
    try {
      const res = await fetchAPI<any>('/ai', {
        method: 'POST',
        body: JSON.stringify({ type: 'interview-prep', job_id: app.job_id, company: app.job?.company_name || '', role: app.job?.title || '', stage: 'hr' }),
      });
      if ((res as any).success && (res as any).data) setAiPrep((res as any).data);
      else setAiPrepError((res as any).error?.message || 'Could not generate prep.');
    } catch { setAiPrepError('Network error — please try again.'); }
  ...
  return (
    <AppShell>
      <div className="max-w-3xl">
        ...
        <p className="text-xs text-slate-500 mb-4">Common HR screening questions and tips. Use AI prep for company-specific coaching.</p>
        ...
        {!isLoading && hrApps.length > 0 && (
          <Card padding="lg" className="mb-4 border-karmio-200 dark:border-karmio-800/50">
            ...
            <div className="space-y-2">
              {hrApps.slice(0, 5).map((app: any) => (
                <div key={app.id} className="flex items-center justify-between py-2 border-b border-slate-100 dark:border-slate-800 last:border-b-0">
                  <div>
                    <p className="text-sm text-slate-700 dark:text-slate-300">{app.job?.company_name || 'Unknown'} — {app.job?.title || 'Unknown'}</p>
                    <Badge variant="info" className="mt-0.5">{app.status.replace('_', ' ')}</Badge>
                  </div>
                  {limits.has_interview_prep ? (
                    <Button size="sm" variant="ghost" onClick={() => handleGeneratePrep(app)}><Sparkles size={12} /> AI Prep</Button>
                  ) : (
            ...
            {!limits.has_interview_prep && <div className="mt-3"><UpgradePrompt feature="AI interview prep" tierNeeded="Pro" /></div>}
          </Card>
        )}

        <WhyHelper className="mb-6">
          These are the most commonly asked HR screening questions. Practice answering each one out loud. Tailor your answers to each specific company and role.
          {user?.country === 'US' && (
            <span className="block mt-2 font-medium">Tip for visa holders: Be prepared to address your work authorization status confidently and concisely.</span>
          )}
        </WhyHelper>
        ...
        <div className="space-y-2">
          {HR_QUESTIONS.map((item, i) => (
            <div key={i} className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700/50 rounded-xl overflow-hidden">
              <button onClick={() => setExpandedQ(expandedQ === i ? null : i)} className="w-full flex items-center justify-between p-4 text-left">
                <div className="flex items-center gap-3">
                  <span className="w-6 h-6 rounded-full bg-karmio-100 dark:bg-karmio-900 flex items-center justify-center text-[10px] font-medium text-karmio-600 dark:text-karmio-400 flex-shrink-0">{i + 1}</span>
        ...
        <Modal open={!!aiPrepApp} onClose={() => { setAiPrepApp(null); setAiPrep(null); setAiPrepError(null); }} title={`AI Prep: ${aiPrepApp?.job?.company_name || ''}`} size="lg">
          {aiPrepLoading ? (
            ...
            <div className="py-6 text-center">
              ...
              <Button variant="secondary" size="sm" onClick={() => aiPrepApp && handleGeneratePrep(aiPrepApp)}>Try again</Button>
            </div>
          ) : aiPrep ? (
            <div className="space-y-4">
              {aiPrep.company_overview && <div><p className="text-xs font-medium text-slate-600 dark:text-slate-400 mb-1">Company Overview</p><p className="text-sm text-slate-700 dark:text-slate-300 leading-relaxed">{aiPrep.company_overview}</p></div>}
              {aiPrep.tailored_questions?.length > 0 && (
                <div>
              ...
              {aiPrep.talking_points?.length > 0 && (
                <div>
                  ...
                  {aiPrep.talking_points.map((point: string, i: number) => (
                    <p key={i} className="text-xs text-slate-600 dark:text-slate-400 flex gap-2"><span className="text-karmio-500 flex-shrink-0">•</span> {point}</p>
                  ))}
                </div>
              )}
              {typeof aiPrep === 'string' && <p className="text-sm text-slate-700 dark:text-slate-300 leading-relaxed whitespace-pre-wrap">{aiPrep}</p>}
            </div>
          ) : (
